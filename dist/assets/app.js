const STORAGE_KEY='salescost-pro-v9-revenue-cost';
...
function openRevenueTargetModal(){window.activeFormKey='revenueTarget';document.getElementById('modalTitle').textContent='Tạo doanh thu mục tiêu';document.getElementById('recordForm').innerHTML=`<div class="field"><label>Người phụ trách<input name="employee" value="Trần Diệu Hương" required></label></div><div class="field"><label>Bộ phận<input name="department" value="Bộ phận Kinh doanh" required></label></div><div class="field"><label>Kỳ mục tiêu<select name="period"><option>Tháng 8/2026</option><option>Quý 3/2026</option><option>Năm 2026</option></select></label></div><div class="field"><label>Doanh thu mục tiêu<input name="target" type="number" required></label></div><div class="field full"><label>Ghi chú<textarea name="note"></textarea></label></div>`;document.getElementById('modalBackdrop').style.display='grid'}
